...
<body>
    <h1>지점 목록</h1>
    <a href="{% raw %}{% url 'market_create' %}{% endraw %}">새 지점 추가</a>

    <!-- 리스트 안에 있는 객체를 하나씩 꺼내서 반복 -->
    {% raw %}{% for test_market in test_market_list %}
    <div class="market-item">
        <h3>{{ test_market.name }}</h3>
        <p>{{ test_market.description }}</p>
        <p>위치: {{ test_market.location }}</p>
    </div>
    {% endfor %}{% endraw %}
</body>
</html>
